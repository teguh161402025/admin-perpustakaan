...
const Pengambilan = () => {
        ...
        const getDataPeminjaman = async () => {
            try {
                const data = onSnapshot(
                    query(collection(db, 'peminjaman'), where('status', '==', 'Disetujui')),
                    async (querySnapshot) => {
    ...
    const handleConfirmation = async (id, durasi, id_book) => {
        ...
        try {
            ...
            toast.current.show({ severity: 'success', summary: 'Sukses', detail: 'Buku Telah Diambil' });
    ...
    const confirmationTemplate = (data) => {
        return <div onClick={() => handleConfirmation(data.id, data.durasi, data.id_buku)} className='bg-blue-500 py-2 text-center text-white px-8 cursor-pointer hover:transition-color hover:bg-blue-700 transition ease-in-out duration-500 w-44 rounded-md'>Tandai Sudah Diambil</div>;
    };
    const header = (
...
export default withAuth(Pengambilan)